...
class authController {
    ...
    async checkAuth(req, res, next) {
        try {
            if (req.isAuthenticated())
                return res.status(200).send({ message: "کاربر با موفقیت احراز هویت شده است." });
    ...
    async signup(req, res, next) {
        try {
            await authServices.checkUserInput(req.body);
    ...
    async sendOtp(req, res, next) {
        try {
            ...
            return res.status(200).send({ message: "کد راستی آزمایی با موفقیت ارسال شد." });
        } catch (e) {
            console.error(e);
            return res.status(500).send({ message: "خطای سرور." });
        }
